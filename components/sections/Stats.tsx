...
export function Stats() {
  ...
  return (
    <section ref={sectionRef} aria-label={t.heading} className="py-20 lg:py-28">
      <Container>
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          <div>
            ...
            <h2 className="h1 font-semibold text-fg mt-6 lg:mt-8 max-w-xl">{t.heading}</h2>

            <p className="body-lg text-fg-muted mt-5 max-w-xl">{t.description}</p>

            <div className="mt-10 lg:mt-14 max-w-xl">
              <p className="text-fg-muted text-[0.9375rem]">
                <span className="font-medium text-fg">{t.evaluatedStrong}</span>
                {t.evaluatedRest}
              </p>
              ...
              <div className="flex items-center gap-4 mt-6">
                ...
                <span className="text-fg-muted text-sm">
                  <span className="text-fg font-medium">—</span> {t.rated}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <article className="bg-surface-alt rounded-3xl p-7 lg:p-8 lg:min-h-[260px] flex flex-col lg:justify-between">
                <span className="inline-flex items-center gap-2 text-fg-muted text-sm">
                  ...
                  {t.cards.matching.label}
                </span>
                <div className="mt-8 lg:mt-0">
                  <div className="font-mono text-5xl lg:text-6xl font-medium text-fg leading-none tracking-tight">
                    <SlotNumber value={47} />
                  </div>
                  <p className="text-fg-muted text-[18px] lg:text-sm mt-2 lg:mt-4">{t.cards.matching.caption}</p>
                </div>
              </article>
              ...
              <article className="bg-surface-alt rounded-3xl p-7 lg:p-8 lg:min-h-[260px] flex flex-col lg:justify-between">
                <span className="inline-flex items-center gap-2 text-fg-muted text-sm">
                  ...
                  {t.cards.success.label}
                </span>
                <div className="mt-8 lg:mt-0">
                  <div className="font-mono text-5xl lg:text-6xl font-medium text-fg leading-none tracking-tight">
                    <SlotNumber value={92} />%
                  </div>
                  <p className="text-fg-muted text-[18px] lg:text-sm mt-2 lg:mt-4">{t.cards.success.caption}</p>
                </div>
              </article>
            </div>
          </div>
