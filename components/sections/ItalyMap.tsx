...
import { DEFAULT_REGION, REGIONS } from '@/lib/data/regions';
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          <div className={`im-detail min-w-0${changing ? ' is-changing' : ''}`}>
            ...
            <dl className="grid grid-cols-[max-content_max-content] gap-x-16 lg:grid-cols-4 lg:gap-x-5 gap-y-6 mb-8 lg:mb-10">
              {[
                [formatK(region.stats.companies), 'Companies'],
                [formatK(region.stats.employed), 'Employed'],
                [region.stats.gdpShare, 'of GDP'],
                [region.stats.exports2023, 'Exports 2023'],
              ].map(([value, label]) => (
                <div key={label}>
            ...
            <div className="mb-8 lg:mb-10">
              <h3 className="text-xl font-semibold text-fg mb-[18px]">Key sectors</h3>
              <TagCloud id="imIndustries" tags={region.industries} mobile={isMobile} className="flex flex-wrap gap-2 max-w-md -ml-1" />
            </div>
            <div>
              <h3 className="text-xl font-semibold text-fg mb-[18px]">Industrial districts</h3>
          ...
          >
            ...
            <div className="im-tt-stats">
              <span>
                <strong>{hoveredRegion ? formatK(hoveredRegion.stats.companies) : ''}</strong> companies
              </span>
              <span>
                <strong>{hoveredRegion?.stats.gdpShare}</strong> of GDP
              </span>
            </div>
            <div className="im-tt-hint">{hoveredRegion ? (hoveredRegion.tier === 1 ? 'Click for details' : 'Click to view') : ''}</div>
          </div>
